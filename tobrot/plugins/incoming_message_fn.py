...
async def g_yt_playlist(client, message):
    ...
    if message.reply_to_message:
        ...
        await message.reply_text("<code>No downloading source provided</code>", quote=True)
        return
    if "youtube.com/playlist" in url:
        i_m_sefg = await message.reply_text(
            f"<b>{usr_mention}\nYour Request has been ADDED</b>\n<code>This process running on backgroud. Please wait until Upload</code>\n<i>Process take some time, so don't cancel or delete your leech</>\n#CendrawasihLeech",
            parse_mode="html",
        )
...
async def rename_tg_file(client, message):
    ...
    if not message.reply_to_message:
        await message.reply("<code>No downloading source provided</code>", quote=True)
        return
